    ...
    if( TargetSmth.IsItem() )
    {
		...
		if( item && item->IsDrawContour() && item->SprDrawValid )
		{
			...
			if (GameOpt.ShowContourItems)
			{
				...
				if (item->Proto->ColorContour>0 && item->ColorContour==0)
				{
					int Color=item->Proto->ColorContour;
				...
				else if (item->Proto->ColorContour==0 && item->ColorContour==0)
				{
        ...
        if( item && item->IsDrawContour() && item->SprDrawValid )
		{
			...
			if (GameOpt.ShowContourItems)
			{
				...
				else if( proto->ColorContour == 0 && item->ColorContour == 0 )
				{
					item->SprDraw->SetContour(CONTOUR_CUSTOM, COLOR_CONTOUR_YELLOW );
					item->SprDraw->SetColor( 0 );
				}
			...
			else if( proto->ColorContour != -1 && item->ColorContour != -1 )
			{
				item->SprDraw->SetContour( 0 );
			}
		}
    ...
    if( !LMenuTryActivated && Timer::FastTick() - GameMouseStay >= LMENU_SHOW_TIME )
    {
        ...
        if( TargetSmth.IsItem() )
        {
            ...
            if( item && item->IsDrawContour() && item->SprDrawValid )
			{
				...
				{
					if( proto->ColorContour != -1 && item->ColorContour != -1 )
				...
				{
													-1							50
					if (proto->ColorContour!=-1 && item->ColorContour==0)
					{
						int n1, n2, n3, n4, n5, n6, n7, n8, n9;
						n1 = proto->ColorContour / 100000000;
						n2 = (proto->ColorContour - n1 * 100000000) / 10000000;
